...
class Meter {
  ...
  constructor({
    ...
    this.hierarchy.forEach((h, i) => {
      ...
      if (i === 0) {
        if (typeof h === 'number') {
          const tempo = 60 * h / this.duration;
          const ps = new PulseStructure({
            tempo,
            size: h,
            startTime: this.startTime,
          ...
          const summed = sum(h);
          const tempo = 60 * summed / this.duration;
          const beatDur = this.duration / summed;
          h.forEach((subH, j) => {
            const startTime = this.startTime + beatDur * sum(h.slice(0, j));
            const ps = new PulseStructure({
              tempo,
              size: subH,
              startTime,
        ...
        if (typeof h === 'number') {
          this.pulseStructures[i - 1].forEach(parentPS => {
            parentPS.pulses.forEach((p, j) => {
              duration = parentPS.pulseDur;
              const tempo = 60 * h / duration;
              const startTime = p.realTime + parentPS.pulseDur * j;
              const ps = PulseStructure.fromPulse(p, duration, h, { layer: i })
              subPulseStructures.push(ps)
            })
          })
        } else {
          ...
          this.pulseStructures[i - 1].forEach(parentPS => {
            parentPS.pulses.forEach((p, j) => {
              ...
              h.forEach((subH, k) => {
                const startTime = p.realTime + beatDur * sum(h.slice(0, k));
...
const a = new Meter({ hierarchy: [4, 4] });

const firstPulse = a.pulseStructures[0][0].pulses[0];
const firstPulseInside = a.pulseStructures[1][0].pulses[0];
console.log(firstPulse)
console.log(firstPulseInside)
console.log(firstPulse.uniqueId === firstPulseInside.uniqueId)
// console.log(a)
export { 
  Pitch,
  Trajectory,
